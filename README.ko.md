...
*메인 창 — 파일 이름을 입력하고 Enter.*

<img src="docs/2.png" width="700" alt="App menu">

*앱 메뉴 — 설정, 필터, 언어, 검색 도움말 등.*
...
- 결과 활용 — 우클릭(열기 / 폴더에서 보기 / 이 폴더에서만 검색 / 경로 복사 / 삭제),
  탐색기로 드래그 앤 드롭 (복사, 이동), CSV 내보내기, 선택 항목 개수·총 크기 표시
- 안 지워지던 항목 삭제 — 이름 끝에 공백이나 점이 있거나 경로가 260자를 넘어 탐색기로는 다루지 못하는 파일·폴더를 바로 삭제. 삭제가 실패하면 그 원인과 파일을 사용 중인 프로그램을 표시
...
- 전역 단축키 `Win + Alt + S` — 다른 앱 사용 중에도 창을 즉시 불러옴 (치울 때는 Esc · 설정에서 끌 수 있음)
- 다크 테마, 7개 언어 지원: 한국어 · 영어 · 중국어(간체) · 일본어 · 스페인어 · 독일어 · 프랑스어 (첫 실행 시 Windows 표시 언어에 맞춰 시작)
...
| `IMG_*_편집.txt` | 이름 전체가 그 모양인 것 (확장자 포함 · 아무 확장자면 `IMG_*_편집*`) |
| `프로젝트 folder:` | 폴더만 찾기 (파일만 찾을 때는 `file:`) |

자세한 설명은 앱 메뉴(HdrTracer ▼) → 검색 도움말에 있음.

### 북마크 (고정 검색)

검색창을 클릭하면 최근 검색 목록이 뜸. 항목의 📌 버튼으로 고정하면 목록 맨 위에
계속 유지되고, 위에서부터 `Ctrl+1~9`로 즉시 실행할 수 있음.

### 탐색기로 끌어 놓기

선택한 결과를 탐색기 창이나 바탕화면으로 끌어 놓으면 **복사**됨.
받는 곳이 같은 드라이브여도 원본은 그대로 남음.

한 가지 주의: 여러 항목을 **Shift 키를 누른 채로 끌면 단순 이동이 됨**.
Shift로 고른 뒤에는 키에서 손을 떼고 끌면 복사가 됨. Ctrl 키를 누른 채로 고르거나
하나만 클릭한 경우는 해당 없음.

### 파일 삭제

결과를 우클릭 → **휴지통으로 삭제**. 평범한 파일은 휴지통으로 가므로 복구할 수 있음.

이름 끝에 공백이나 점이 있거나 경로가 260자를 넘는 항목은 휴지통으로 보낼 수 없음.
이런 항목은 별도 확인을 받은 뒤 영구 삭제함. **확인 없이 영구 삭제되는 경우는 없으며**,
시스템 보호 경로(Windows, Program Files 등)는 영구 삭제로 올리지 않음.

삭제가 실패하면 그 원인 — 다른 프로그램이 사용 중, 권한 없음, 이름·경로 비정상 등 —
과 함께 그 항목을 사용하고 있는 프로그램의 이름과 PID를 표시함.
...
- 탐색기로 끌어 놓으면 복사됨. Shift를 누른 채 놓으면 이동이 되는데,
  이는 앱이 바꿀 수 없는 Windows 규칙임.
- 코드 서명이 없어서 SmartScreen 경고가 나타남.
- 삭제는 기본적으로 휴지통으로 이동함. 휴지통으로 보낼 수 없는 항목만 별도 확인을 받은 뒤
  영구 삭제됨. 시스템 폴더 등 중요한 경로가 포함되면 삭제 전 한 번 더 경고함.
- 잠긴 파일과 소유권이 없는 파일은 삭제할 수 없음. 대신 그 원인을 표시함 —
  소유권을 강제로 바꾸거나 프로세스를 강제 종료하지 않음.
...
[Flaticon](https://www.flaticon.com/)에서 받았음 (출처 표기 조건의 무료 사용).
